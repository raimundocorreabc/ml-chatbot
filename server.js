...
if (!SHOPIFY_STORE_DOMAIN || !SHOPIFY_STOREFRONT_TOKEN) {
  throw new Error('Falta SHOPIFY_STORE_DOMAIN o SHOPIFY_STOREFRONT_TOKEN');
}
...
`,
  ...
  return (d.collections?.edges || []).map((e) => ({
    title: e.node.title,
...
`,
  ...
  return (d.search?.edges || []).map((e) => {
    ...
    const compareCurrency =
      p.compareAtPriceRange?.minVariantPrice?.currencyCode ?? currency;
...
`,
      ...
      const items = (d.collectionByHandle?.products?.edges || []).map((e) => {
        ...
        return {
          ...
          compareCurrency:
            ...
            (p.priceRange?.minVariantPrice?.currencyCode ?? 'CLP'),
        };
      });
      ...
      console.warn('[tops] error colección', err?.message || err);
    }
  }
...
`,
    ...
    console.warn('[tops] error global', err?.message || err);
  }
...
const SHIPPING_ZONES = [
  ...
  },
  {
    zone: 'ZONA NORTE',
    cost: 10990,
    regions: ['Arica y Parinacota', 'Tarapacá', 'Tarapaca', 'Antofagasta', 'Atacama'],
  },
...
function surfaceQueryBoost(surface) {
  switch (surface) {
    case 'bano':
      return ['limpiador baño', 'antihongos baño', 'quita sarro baño', 'desinfectante baño', 'wc'];
    case 'cocina':
      return ['desengrasante cocina', 'limpiador cocina', 'antigrasa cocina', 'acero inoxidable cocina'];
    case 'alfombra':
      return ['limpiador alfombra', 'tapiceria', 'quitamanchas alfombra', 'dr beckmann'];
    case 'pisos':
      return ['limpiador pisos', 'bona', 'lithofin', 'abrillantador pisos'];
    case 'vidrio':
      return ['limpia vidrios', 'glass cleaner', 'antiempañante vidrio'];
    case 'madera':
      return ['limpiador madera', 'acondicionador madera', 'abrillantador madera'];
    case 'acero':
      return ['limpiador acero inoxidable', 'weiman acero', 'polish acero'];
    case 'ropa':
      return ['quitamanchas ropa', 'blanqueador ropa', 'detergente capsulas'];
    case 'ollas':
      return [
        'pasta multiuso',
        'pink stuff pasta',
    ...
    case 'azulejos':
      return ['antihongos', 'mildew away', 'moho', 'limpiador juntas', 'limpiador azulejos', 'ceramica'];
    default:
      return [];
  }
}
...
function buildPreciseQueriesForSegment(phrase) {
  ...
  const isWCDeo =
    ((/\b(desodorante|neutralizador|aromatizante|spray)\b/.test(nq) && /(\bwc|bañ|bano)\b/.test(nq)) ||
...
async function aiProductQuery(userText) {
  try {
    ...
    const keywords = Array.isArray(parsed.keywords)
      ? parsed.keywords
          .map((s) => singularize(norm(String(s).trim())))
          .filter(Boolean)
    ...
    const brands = Array.isArray(parsed.brands)
      ? parsed.brands.map((s) => String(s).trim()).filter(Boolean).slice(0, 3)
      : [];
...
async function findHandleForStock(message = '', meta = {}) {
  ...
  const list = (candidateList.length ? candidateList : scored).sort((a, b) => {
...
function pluralDisponible(n) {
  return Number(n) === 1 ? 'disponible' : 'disponibles';
}
...
/* ----- Intents ----- */
const PURPOSE_REGEX =
  /\b(para que sirve|para qué sirve|que es|qué es|como usar|cómo usar|modo de uso|instrucciones|paso a paso|como limpiar|cómo limpiar|consejos|tips|guia|guía|pasos)\b/i;
...
function detectIntent(text = '') {
  ...
  if (/(categorias|categorías|tipos de productos|colecciones|que productos venden|qué productos venden)/.test(q))
    return 'categories';
...
function parseBrandCarouselConfig() {
  try {
    return JSON.parse(BRAND_CAROUSEL_JSON || '');
  } catch {
    return [];
  }
}
...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (!message) {
      return res.json({
        text: '¿En qué te ayudo? Puedo sugerirte productos, formas de uso o calcular envío por región.',
      });
    }
    ...
    if (intent === 'stock') {
      ...
      if (info.total !== null) {
        ...
        if (withQty.length === 1) {
          const v = withQty[0];
          const label = isDefaultVariantTitle(v.title)
            ? '**Stock disponible:**'
            : `**Variante ${v.title} — Stock:**`;
          return res.json({
    ...
    if (intent === 'brands') {
      ...
      const vendors = (d.products?.edges || [])
        .map((e) => String(e.node.vendor || '').trim())
        .filter(Boolean);
    ...
    if (intent === 'shipping') {
      const header =
        FREE_TH > 0 ? `En **RM** hay **envío gratis** sobre **${fmtCLP(FREE_TH)}**.` : 'Hacemos despacho a **todo Chile**.';
    ...
    if (intent === 'shipping_region') {
      ...
      if (COMUNAS_F.has(fold(q))) {
        return res.json({
          text: `Despachamos a **todo Chile**. Para **${q}**, ingresa tu **región/comuna** en el checkout y verás el costo exacto. Si me dices tu **región**, te doy el costo referencial.`,
        });
      }
    }
    ...
    if (intent === 'points') {
      ...
      return res.json({
        text: `**Mundopuntos**: ganas **${earn} punto(s) por $1**. Canje: **100 puntos = ${fmtCLP(redeem100)}**. ${
          url ? `Más info: ${url}` : 'Adminístralo en el widget de recompensas.'
        }`,
      });
    }
    ...
    if (intent === 'shopping') {
      ...
      if (picks && picks.length) {
        return res.json({
          text: `Te dejo una opción por ítem (con precio y descripción real):\n\n${await buildProductsMarkdown(picks)}`,
        });
      }
      ...
      try {
        ...
        const productContext = topPick
          ? [
              ...
              `- compareAt: ${
                topPick.compareAt != null ? fmtMoney(topPick.compareAt, topPick.compareCurrency || topPick.currency) : ''
              }`,
...
`;
      ...
      const greetLine =
        meta?.userFirstName &&
        meta?.tipAlreadyShown !== true &&
        Number(meta?.cartSubtotalCLP || 0) < Number(FREE_TH || FREE_TH_DEFAULT)
          ? `Hola, ${meta.userFirstName} 👋 | Te faltan ${fmtCLP(
              Number(FREE_TH || FREE_TH_DEFAULT) - Number(meta?.cartSubtotalCLP || 0)
            )} para envío gratis en RM.`
          : '';

      const tipBlock = greetLine || tipText ? `TIP:\n${[greetLine, tipText].filter(Boolean).join('\n')}` : '';
    ...
    return res.json({
      text: '¿Me cuentas un poco más? Puedo sugerirte productos o calcular envío por región.',
    });
  } catch (e) {
    console.error(e);
